...
Unordered list - unordered list - unordered list - unordered list - unordered list - unordered list - unordered list - unordered list - unordered list - unordered list - unordered list - unordered list - unordered list - unordered list:

- First item First item First item First item First item First item First item First item First item First item First item First item First item
- Second item
...
Same image via the custom `<Image>` component with the card template:

<Image src="/blog/2026/04/24-resumed-session.webp" alt="Resumed session screenshot" template="card">
  Card-template rendering wraps the image in a rounded container with shadow and an optional caption. This caption text comes from the component's children.
</Image>

Without the card template (bare image, full-width):
...
Outline variant — visual chrome (border, rounded corners, padding) without a behavior:

<Button variant="outline">Click me</Button>

As an anchor — `as="a"` with `href` makes the button navigate. The dynamic render path doesn't permit inline event handlers, so anchor-style buttons are how interactive buttons land on R2-backed entries:

<Button as="a" href="/blog" variant="outline">Go to blog</Button>
...
The interactive collapsible UI is authored as standard `<details>` / `<summary>` HTML — Nextra maps `<details>` to its `Details` component, which uses `Collapse` internally for the animation. Click the summary to toggle:
