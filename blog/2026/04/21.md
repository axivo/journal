...
1. **Worker KV namespace purge:** The Worker purges keys from the previous build via `/__internal/purge-kv-cache`, called by the deploy script with a shared secret. Using the Worker's own KV binding keeps the API token out of CI.
