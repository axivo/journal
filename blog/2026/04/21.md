...
Algolia [DocSearch](https://docsearch.algolia.com) follows the same discipline. The initial page ships only a small trigger button. The full DocSearch modal is lazy-loaded via dynamic `import()` the first time a reader opens search, so the search UI's bundle never lands on pages nobody searched on.
