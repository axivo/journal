...
The AXIVO website runs on Cloudflare's edge for roughly the price of a coffee a month. The pieces are conventional — Next.js, Nextra, OpenNext, R2, Workers KV — but the way they cooperate is not. This post is about the cooperation.

The site is a [Next.js](https://nextjs.org) application using the [Nextra](https://nextra.site) docs theme, statically generated at build time, deployed to Cloudflare Workers via [OpenNext](https://opennext.js.org). Content lives outside the deployment artifact in R2. Listings, tag pages, and individual entries render at the edge from MDX bodies fetched at request time. The Worker barely runs.

That last sentence is the headline. The rest of the post is why.
...
We measured. KV reads from a warm edge averaged 2-4× faster than R2 for the same payload sizes. The difference is structural: R2 reads cross a regional boundary even at warm cache; KV reads stay local. For the page-rendering hot path, latency compounds — the difference between rendering a listing page in 80ms and 200ms shows up in p99 user-facing TTFB.
...
The architectural property KV gives is "writes once on cold miss, reads from any PoP forever, deletes only on deploy." That matches exactly what we want: every page rendered once, stays rendered for every future visitor everywhere, until the next deploy flushes the namespace.

## The Metadata Manifest

Listing every post on a tag page or an index page naively means listing every object under a prefix on every request. R2 calls those Class A operations. Doing them per request is wasteful, slow, and bills you twice — once for the list, once for each metadata fetch.
...
1. **The currently-deployed Worker purges its own KV namespace.** The deploy script calls `/__internal/purge-kv-cache` on the live Worker with a shared secret. The Worker uses its own KV binding to list and delete every key from the previous build. This is the part most people get wrong: the _current_ Worker has the binding and the keys, the _new_ Worker doesn't know what to clean up. Purging before deploying is correct, and routing it through the live Worker means no API token in the deploy environment.
...
4. **Warming.** Fetches `/sitemap.xml`, filters to URLs at path depth ≤ 2 (section roots and listings), issues parallel GETs. The Worker renders them, populates `caches.default` and the zone cache. Smart Tiered Cache propagates warm state to other PoPs on first miss. Individual entries cache on first-visitor demand.
...
## The Bundle and Cache Invalidation
