...
Listing every post on a tag page or an index page naively means listing every object under a prefix on every request. R2 calls those Class A operations. Doing them per request is slow and bills twice — once for the list, once for each metadata fetch.
